...
class ScreensaverManager:
    ...
    def __init__(self):
        ...
        self.auto_scroll_time = 60.0  # 1 minute between effects
        self.last_scroll_time = time.monotonic()
        
        # Effects list (6 effects - ultra stable)
        self.effects = [
            PlasmaEffect(),
            SpiralEffect(),
            MatrixEffect(),
            # ColorMatrixEffect(),  # Removed - memory allocation crashes
            JuliaFractalEffect(),
            # FireEffect(),  # Removed - only uses small portion of screen
            SineWaveEffect(),
            # StarfieldEffect(),  # Removed - too memory intensive
            # StreamersEffect(),  # Removed - memory limit reached
        ]
        ...
        print("⏰ Auto-scrolling every 1 minute!")
        print("🎬 Effects: Plasma → Spiral → Matrix → Julia Fractal → Sine Waves → repeat")
        print("💾 Ultra stable - 5 effects total (rock solid, no crashes)")
...
class MatrixEffect:
    """Green Matrix with real text characters"""
    
    def __init__(self):
        print("💚 Creating green Matrix text effect...")
        ...
        self.char_width = 9
        self.char_height = 12
        self.columns = 35  # Use 35 columns as requested
        ...
        # Text labels for characters (keep it small)
        self.active_labels = []
        ...
        for i in range(self.columns):
            ...
            self.drop_speeds[i] = random.choice([0.5, 1, 1.5])
        ...
        gc.collect()
        print(f"💚 Green Matrix text created - Free memory: {gc.mem_free()} bytes")
    ...
    def update(self):
        """Update green Matrix text effect"""
        self.time += 0.1
        ...
        # Conservative memory cleanup - only 15 labels max
        if len(self.active_labels) > 15:
            labels_to_remove = self.active_labels[:8]  # Remove oldest 8
        else:
            # Normal cleanup
            labels_to_remove = []
            for label_obj in self.active_labels:
                if hasattr(label_obj, '_matrix_age'):
                    ...
                    if label_obj._matrix_age > 25:  # Short lifespan
        ...
        # Update each column's drop - use all 35 columns
        for col in range(self.columns):
            if self.drop_active[col]:
                drop_y = int(self.drop_positions[col])
                
                # Add new character (very conservative)
                if 0 <= drop_y < self.rows and random.randint(0, 12) == 0:  # Very rare spawn
                    ...
                    try:
                        text_label = label.Label(
                            terminalio.FONT,
                            text=char,
                            color=self.matrix_green,  # Always green
                            x=col * self.char_width,
                            y=drop_y * self.char_height + 8
                        )
                        text_label._matrix_age = 0
                ...
                if self.drop_positions[col] > self.rows + 5:
                    ...
                    self.drop_speeds[col] = random.choice([0.5, 1, 1.5])
                    self.drop_active[col] = random.choice([True, True, False])
            else:
                # Randomly reactivate drops
                if random.randint(0, 200) < 2:  # Very low chance
...
class ColorMatrixEffect:
    ...
    def update(self):
        ...
        # Memory cleanup with more room now
        if len(self.active_labels) > 40:  # Higher limit since starfield is gone
            labels_to_remove = self.active_labels[:15]  # Remove oldest 15
        else:
            # Normal cleanup
            labels_to_remove = []
            for label_obj in self.active_labels:
                if hasattr(label_obj, '_matrix_age'):
                    label_obj._matrix_age += 1
                    if label_obj._matrix_age > 30:  # Remove after 30 frames (shorter)
                        labels_to_remove.append(label_obj)
        ...
        # Update each column's drop (expanded now that starfield is removed)
        for col in range(min(self.columns, 30)):  # Increased back up since we have more memory
            if self.drop_active[col]:
                drop_y = int(self.drop_positions[col])
                
                # Add new character at drop position
                if 0 <= drop_y < self.rows and random.randint(0, 5) == 0:  # Moderate frequency
                    char = random.choice(self.characters)
